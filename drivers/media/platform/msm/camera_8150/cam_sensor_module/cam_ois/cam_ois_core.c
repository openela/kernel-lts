...
struct cam_sensor_i2c_reg_setting_array bu63169_pll_settings = {
    .reg_setting =
	{
		...
		{.reg_addr = 0x8263, .reg_data = 0x9F05, .delay = 0x00, .data_mask = 0x00}, \
	...
	struct i2c_settings_array *i2c_set)
{
	...
	if (MASTER_0 == o_ctrl->io_master_info.cci_client->cci_i2c_master && 19696 != get_project()) {
...
static int cam_ois_pkt_parse(struct cam_ois_ctrl_t *o_ctrl, void *arg)
{
	...
	if (19696 == get_project()) {
		...
		//0x1000  OIS_SET_THREAD_STATUS_MASK
		//0x01 OIS_SET_MIAN_GET_HALL_DATA_THREAD_MASK
		//0x02 OIS_SET_TELE_GET_HALL_DATA_THREAD_MASK
	   // send a dummy configure informaintion for set ois get hall thread
	   if (ioctl_ctrl->reserved & 0x1000) {
		   if (ioctl_ctrl->reserved & 0x01){
			   is_enable_main_ois_thread = true;
		   }
		   if (ioctl_ctrl->reserved & 0x02){
			   is_enable_tele_ois_thread = true;
		   }
		   set_ois_thread_status(is_enable_main_ois_thread,is_enable_tele_ois_thread);
		   CAM_DBG(CAM_OIS,"set ois get hall data thread status : main:[%d] tele: [%d]",
...
int cam_ois_driver_cmd(struct cam_ois_ctrl_t *o_ctrl, void *arg)
{
	...
	case CAM_GET_OIS_HALL_POSITION: {
		...
		if (MASTER_0 == o_ctrl->io_master_info.cci_client->cci_i2c_master) {
			if(19696 == get_project()) {
				/*Add by Wangchao @ RM camera for test*/
				CAM_ERR(CAM_OIS, "Test for SEM1215S");
				hall_x_addr = 0x0B10;
				hall_y_addr = 0x0B12;
			} else {
				hall_x_addr = 0x843f;
	...
	case CAM_GET_OIS_EIS_HALL: {
		if (19696 == get_project()) {
			...
			if (o_ctrl->cam_ois_state == CAM_OIS_START && strstr(o_ctrl->ois_name, "sem1215s_ois") != NULL) {
