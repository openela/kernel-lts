...
void	SetSinWavGenInt124( void )
{
	...
	RamWrite32A( SinWave_Offset		,	0x00000000 ) ;		// ­¶ügÌItZbgðÝè
	RamWrite32A( SinWave_Phase		,	0x60000000 ) ;		// ³·gÌÊÊ
	RamWrite32A( SinWave_Gain		,	0x00000000 ) ;		// ­¶ügÌAbel[^(úlÍ0[dB])
//	RamWrite32A( SinWave_Gain		,	0x7FFFFFFF ) ;		// ­¶ügÌAbel[^(úlÍCut)
//	SetTransDataAdr124( SinWave_OutAddr	,	(UINT32)SinWave_Output ) ;		// úlÌoÍæAhXÍA©ªÌo

	RamWrite32A( CosWave_Offset		,	0x00000000 );		// ­¶ügÌItZbgðÝè
	RamWrite32A( CosWave_Phase 		,	0x00000000 );		// ³·gÌÊÊ
	RamWrite32A( CosWave_Gain 		,	0x00000000 );		// ­¶ügÌAbel[^(úlÍCut)
//	RamWrite32A( CosWave_Gain 		,	0x7FFFFFFF );		// ­¶ügÌAbel[^(úlÍ0[dB])
//	SetTransDataAdr124( CosWave_OutAddr	,	(UINT32)CosWave_Output );		// úlÌoÍæAhXÍA©ªÌo
...
//TRACE(" lvl = %08x \n", (unsigned int)UlActChkLvl  ) ;
	...
	MesFil124( NOISE ) ;					// ªèptB^[ðÝè·éB
...
UINT8	RunGea124( void )
{
	...
	MesFil124( THROUGH ) ;				// ªèptB^[ðÝè·éB
...
////////// ////////// ////////// ////////// //////////
	...
	RamWrite32A( CMD_GYRO_WR_ACCS , 0x1B180000 );		/* FS_SEL=3Åè & Disable Self Test */
	UcStRd = 1;
	while( UcStRd ) {
		UcStRd = RdStatus124(1);
	}
...
////////// ////////// ////////// ////////// //////////
	...
	RamWrite32A( CMD_GYRO_WR_ACCS , 0x1BD80000 );		/* FS_SEL=3Åè & Enable Self Test */
	UcStRd = 1;
	while( UcStRd ) {
		UcStRd = RdStatus124(1);
	}
...
//TRACE("UsGyrYval = %04x\n", UsGyrYval ) ;
	...
	RamWrite32A( CMD_GYRO_WR_ACCS , 0x1B000000 | ( UlGyroConfig >> 8));		/* ³ÌÝèlÉß· */
	UcStRd = 1;
	while( UcStRd ) {
		UcStRd = RdStatus124(1);
	}
...
// History			: First edition
//********************************************************************************
/*  bit7  	HX GYR			Hall X  Æ¯ûüÌGyroMªGX?               0:GX  1:GY  */
/*  bit6  	HX GYR pol		Hall X+ Æ¯ûüÌGyroMªX+ÆG+Å¯ûü?   0:NEG 1:POS */
/*  bit5  	HY GYR pol		Hall Y+ Æ¯ûüÌGyroMªY+ÆG+Å¯ûü?   0:NEG 1:POS */
/*  bit4  	GZ pol			î{É«ÉÎµÄGyroZMª¯ûü?            0:NEG 1:POS */
/*  bit3  	HX ACL			Hall X  Æ¯ûüÌAcclMªAX?               0:AX  1:AY  */
/*  bit2  	HX ACL pol		Hall X+ Æ¯ûüÌAcclMªX+ÆA+Å¯ûü?   0:NEG 1:POS */
/*  bit1  	HY ACL pol		Hall Y+ Æ¯ûüÌAcclMªY+ÆA+Å¯ûü?   0:NEG 1:POS */
/*  bit0  	AZ pol			î{É«ÉÎµÄAcclZMª¯ûü?            0:NEG 1:POS */
                      //   top0btm0//
