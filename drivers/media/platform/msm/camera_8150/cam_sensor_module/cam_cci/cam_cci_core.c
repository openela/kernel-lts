...
#ifdef VENDOR_EDIT
/*add by hongbo.dai@Camera 20181109, for support Laser*/
	...
	enum cci_i2c_queue_t queue)
{
	...
	CAM_DBG(CAM_CCI, "wait DONE_for_completion_timeout for master %d queue %d rc=%d",
		master, queue, rc);
	...
	if (rc <= 0) {
		...
		if (rc == 0) {
			read_val = cam_io_r_mb(base + CCI_I2C_M0_Q0_CUR_WORD_CNT_ADDR +
				reg_offset);

			if (read_val == 0) {
				CAM_DBG(CAM_CCI, "Ignore this timeout for master %d queue %d",
					master, queue);
				goto out;
			}
		}
		...
		if (rc <= 0) {
			if (rc == 0) {
				...
				if (read_val > 0) {
					rc = -ETIMEDOUT;
					CAM_ERR(CAM_CCI, "wait for master %d queue %d timeout again");
				} else {
					CAM_DBG(CAM_CCI,
			...
			CAM_DBG(CAM_CCI, "wait for master %d queue %d retry success",
				master, queue);
		}
	}
...
int32_t cam_cci_control_interface(void* control)
{
    ...
    case CAMERA_CCI_READ:
        ...
        if(rc < 0){
            ...
            CAM_ERR(CAM_CCI, "read exp byte=%d", exp_byte);
            for(i=0;i<exp_byte;i++){
                CAM_ERR(CAM_CCI, "read byte=0x%x,index=%d",
